...
class rbm_classicalsolver:
    ...
    def __init__(self,
        ...
        if np_rng is None:
            ...
            ny_rng = numpy.random.RandomState(1234)
    ...
    def prop_up(self,vis):
        """
        Probability of Markov chain step for up propagation (from visible samples to hidden samples)
    ...
    def sample_h_givenv(self,v0_sample):
        """
        Markov chain transition from given visible layer configuration to generate hidden layer configuration
    ...
    def sample_v_givenh(self,h0_sample):
        """
        Markov chain transition from given hidden layer configuration to generate visible layer configuration
        :param h0_sample: given visible layer configurations
        :return v1_sample: hidden layer configurations
        """
        pre_sigmoid_v1,v1_mean = self.propdn(h0_sample)
        # get a sample of the hiddens given their activation
        # Note that theano_rng.binomial returns a symbolic sample of dtype
        # int64 by default. If we want to keep our computations in floatX
        # for the GPU we need to specify to return the dtype floatX
        v1_sample=theano_rng.binomial(size=v1_mean.shape,
                                      n=1,
                                      p=v1_mean,
                                      dtype=theano.config.floatX
                                      )
    ...
    def gibbs_vhv(self,v0_sample):
        """
        Markov chain transition of visible sample->hidden sample->visible sample with Gibbs sampling
        :param v0_sample: given visible layer configuration
        :return v1_sample: visible layer configuration after two steps of Markov chain transition
        """
        pre_sigmoid_h1,h1_mean,h1_sample=self.sample_h_givenv(v0_sample)

        pre_sigmpid_v1,v1_mean,v1_sample=self.sample_v_givenh(h1_sample)
    ...
    def gibbs_hvh(self,h0_sample):
        """
        Markov chain transition of hidden sample->visible sample->hidden sample, with Gibbs sampling
        :param h0_sample: given hidden layer configuration
        :return h1_sample: hidden layer configuration after two steps of Markov chain transition
        """
    ...
    def changing_weight(self,v_sample):
        ...
        but the Ts's is different, but one can sum up Ts's for all s'
         ...
         :param Hamiltonian: Hamiltonian of the physical system we concern, we mainly use
                Hamiltonian.h
        ...
        exponent=-2*v_sample*self.vbias+\
                 T.sum(
                     T.log(T.cosh(self.hbias-self.W_real.T*v_sample+1j*self.W_imag.T*v_sample)),axis=1)-\
                 T.sum(
                     T.log(T.cosh(self.hbias+self.W_real.T*v_sample-1j*self.W_imag.T*v_sample),axis=1)
                 )
    ...
    def variational_energy(self,v1_sample,Hamiltonian):
        """
        function to compute flpping energy for one sample to the given sample, which is sum_s'<s|H|s'>
        ...
        left_shift_sample=theano.map(lambda x:T.concatenate(x[1:],T.shape_padright(Hamiltonian.pdc*x[0]),axis=1),
                                   sequences=v1_sample)


        ess=T.sum(T.mean(-left_shift_sample*v1_sample,axis=0))
        changing_weight=theano.map(fn=self.changing_weight,sequences=v1_sample)
        esp=1/2*Hamiltonian.h*T.mean(changing_weight)
        return ess+esp

    def variational_update(self,Hamiltonian,lr=0.1,persisitent=None,k=1,):
        """
        ...
        if persisitent:
            updates[persisitent]=nv_samples[-1]

        return updates,variational
...
def testRBM_classical():
    ...
    ising1d=Ising1D.Ising1D(nspins,hfield,pbc)
    initial_sample=RandomStreams.binomial(size=(n_sample,nspin),n=1,p=0.5)
    rbm_classical=rbm_classicalsolver(nvisible=nspin,nhidden=alpha*nspin,np_rng=rng,theano_rng=theano_rng)

    updates,variational_energy=rbm_classical.variational_update(ising1d,persisitent=initial_sample,k=15)
    # start-snippet-5
    ...
    for epoch in range(times):
        ...
        mean_cost = train_rbm()
        print("For epoch %d , the variational energy is %f ",mean_cost)
